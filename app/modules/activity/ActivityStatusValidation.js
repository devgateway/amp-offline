...
export default class ActivityStatusValidation {
  ...
  static getStatus(dehydratedActivity, teamMember, rejected) {
    ...
    const isSameWorkspace = teamMember['workspace-id'] === dehydratedActivity[AC.TEAM];
    return WorkspaceHelper.findById(teamMember['workspace-id']).then(workspace => (
      WSSettingsHelpers.findByWorkspaceId(teamMember['workspace-id']).then(workspaceSettings => {
